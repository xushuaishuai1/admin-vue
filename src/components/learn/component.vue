...
export default {
  ...
  components: {
    parentComponent: {
      ...
      template:
        '<div><span>这里是父组件</span>->我自己数据：({{parentData.msg1}})->子组件数据：({{childMsg.msg2}})<input type="button" value="修改子组件数据" @click="changeChild">' +
        '<br/><br/><br/>' +
      ...
      components: {
        childComponet: {
          ...
          template:
            '<div><span>这里是子组件->我自己数据：({{childData.msg2}})->父组件数据：({{parentMsg.msg1}})</span>' +
